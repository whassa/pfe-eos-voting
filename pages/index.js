...
import { Box, Container, Grid, IconButton, Typography, NativeSelect, Stack } from "@mui/material";
      ...
      >
        <Box sx={{ float: "right", display: 'flex' }}>
          <Box sx={{display: 'flex', marginRight: '10px'}}>
            ...
            <NativeSelect defaultValue={'Date'} onChange={(e) => {dispatch({type: types.SORT_CHANGED, value:e.target.value})}}>
          ...
          <IconButton
            ...
            component="span"
            onClick={() => {dispatch( {type: types.VIEW_CHANGED, value:'listView'})}}
          >
          ...
          <IconButton
            ...
            onClick={() => {dispatch( {type: types.VIEW_CHANGED, value:'gridView'})}}
          >
            <GridViewIcon />
          </IconButton>
        </Box>
        ...
        >
          <Grid item md={6} sx={{ width: 1, marginLeft: { xs: "24px" } }}>
            {state.view === 'listView' && <ListView cardOnClick={cardOnClick} resolutions={state.resolutions} />}
            {state.view === 'gridView' && <GridView cardOnClick={cardOnClick} resolutions={state.resolutions} />}
          </Grid>
        </Grid>
        <Box sx={{marginTop: '10px',}}>
          <Stack direction="row"  >
            <IconButton
                ...
                onClick={() => {fetchResolutinInfo(state.sortInfo.page - 1 )}}
              >
                <NavigateBeforeIcon />
              </IconButton>
            <IconButton
